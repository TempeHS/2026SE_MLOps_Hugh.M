...
2. Refactoring the business problem into a machine learning problem
   - Doctors tend to underestimate how Type II diabetes progresses over time. Therefore based on pre-existing patient data, create a ML model that fits historic datasets that can accurately predict the progression of diabetes to mitigate human error in medicine.

3. Defining success metrics
   - The model fits unseen data well based on the training data.

   - The data output is readable to both those with advanced knowledge of medicine and those with basic/no knowledge.

   - Needs to be consistent with existing data sources instead of using its own unique labelling.
